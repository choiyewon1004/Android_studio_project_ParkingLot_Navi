...
//    private final int MENU_REVERSE_GEO = Menu.FIRST + 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 권한이 열려있는지 확인
        if (permission == PackageManager.PERMISSION_DENIED || permission2 == PackageManager.PERMISSION_DENIED || permission3 == PackageManager.PERMISSION_DENIED || permission4 == PackageManager.PERMISSION_DENIED) {
            // 마쉬멜로우 이상버전부터 권한을 물어본다
            if (VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                // 권한 체크(READ_PHONE_STATE의 requestCode를 1000으로 세팅
                requestPermissions(
                        new String[]{Manifest.permission.INTERNET, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.RECORD_AUDIO},
                        1000);
            }
            return;
        }
...
//
//    private void findSameAddressParkinglot(String Gu, String Dong) {
//        if(Dong.contains("신정동")) {
//            findAddressParkinglot(Dong);
//        }
//        else{
//            Location locationA = new Location("pointA");
//
//
//            Toast.makeText(getApplicationContext(), "주차장이 없습니다 : " + Gu + " "+ Dong + "\n근처 주차장으로 안내", Toast.LENGTH_LONG).show();
//            ArrayList<Integer> filter = new ArrayList<>();
//            Double lat = 0.0;
//            Double lng = 0.0;
//
//            for (int i = 0; i < districtData.size(); i++) {
//                if (districtData.get(i).dong.contains(Dong)) {
//                    filter.add(i);
//                }
//            }
//
//            for(int j = 0; j < filter.size(); j ++){
//                if(districtData.get( filter.get(j) ).gu.contains(Gu)){
//                    lat = districtData.get(filter.get(j)).lat;
//                    lng = districtData.get(filter.get(j)).lng;
//                }
//            }
//
//            locationA.setLatitude(lat);
...
//            int index = -1;
//            for (int i = 0; i < dataArr.size(); i++) {
//                if (dataArr.get(i).name.contains("Dong")) {
//                    filter.add(i);
//                }
//            }
//            String addr = "";
//            for(int j = 0; j < filter.size(); j ++){
//                if( dataArr.get( filter.get(j) ).name.contains("Gu") ){
//                    addr = dataArr.get( filter.get(j) ).name;
//                }
//            }
//
//                if (dataArr.get(i).name.contains(addr)) {
//                    Location locationB = new Location("pointB");
//                    locationB.setLatitude(dataArr.get(i).latitude);
//                    locationB.setLongitude(dataArr.get(i).longitude);
//                    float d = locationA.distanceTo(locationB);
